...
function nano.handle_custom_response(session_data, response)
    ...
    if response_type == nano.WebResponseType.CUSTOM_WEB_RESPONSE then
        local block_page = nano_attachment.get_block_page(attachment, session_data, response)
        return kong.response.exit(200, block_page, { ["Content-Type"] = "text/html" })
    end
    ...
    -- NO_WEB_RESPONSE or unknown fallback
    return kong.response.exit(403, { message = "Blocked by Nano Attachment" })
end
